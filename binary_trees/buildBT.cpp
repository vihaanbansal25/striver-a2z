...
public:
    ...
    TreeNode* build(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int, int>& inMap){
        ...
        root->left = build(preorder, preStart + 1, preStart + numLeft, inorder, inStart, rootIdx - 1, inMap);
        root->right = build(preorder, preStart + numLeft + 1, preEnd, inorder, rootIdx + 1, inEnd, inMap);
        return root;
    }
};